...
if (Config.WORKTYPE == 'private') {
    Jesi.adCmd({pattern: 'warn ?(.*)', fromMe: true, desc: Lang.WREPORT}, (async (message, match) => {
    ...
    Jesi.adCmd({pattern: 'warn ?(.*)', fromMe: true, desc: Lang.WREPORT, dontAddCommandList: true}, (async (message, match) => {
        if (match[1] == '' && message.reply_message) {
            let grup = await message.client.groupMetadata(message.jid);
            var jids = [];
            mesaj = '';
