...
let msg = Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*This Plugin is Officially Approved!* ✅' : '*This Plugin is Officially Approved!* ✅'
let unmsg = Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*This Plugin isn\'t Officially Approved!* ❌' : '*This Plugin isn\'t Officially Approved!* ❌'
...
var LANG = {
            unaffinfo: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*Danger Level of Installed Plugin:* _%' : '*Danger Level of Installed Plugin:* _%',
            harmful: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*This Plugin Cannot Be Installed As It Is Harmful!*' : '*This Plugin Cannot Be Installed As It Is Harmful!*',
            duplicate: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*You Cannot Install the Same Plugin 2 Times!*' : '*You Cannot Install the Same Plugin 2 Times!*',
            limit: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*This Plugin Exceeds Security Limit!*\n*Percentage of Harm:* _%' : '*This Plugin Exceeds Security Limit!*\n*Percentage of Harm:* _%',
            imside: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*You Cant Reinstall Existing Plugins!*' : '*You Cant Reinstall Existing Plugins!*'
};
Jesi.adCmd({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {

    if (match[1] == '') return await message.client.sendMessage(message.jid,Lang.NEED_URL + '.install https://gist.github.com/Sl-Yasia/b9cebf33a2c400a75936d69d40aa2424', MessageType.text)
    try {
    ...
    if (response.statusCode == 200) {
        ...
        var plugin_name = response.body.match(/adCmd\({.*pattern: ["'](.*)["'].*}/);
        ...
        try {
            ...
            fs.unlinkSync('/root/Queen-Jesi/plugins/' + plugin_name + '.js')
...
Jesi.adCmd({pattern: 'plugin$', fromMe: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('Sl-Yasia') ? msg : unmsg
...
Jesi.adCmd({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
