...
Jesi.adCmd({pattern: 'speedtest ?(.*)', fromMe: true, desc: Lang.SPEEDTEST_DESC, usage: 'speedtest user // speedtest server'}, (async (message, match) => {
    if (match[1] == 'user' || match[1] == 'User') {
        
        // Preliminary Message
        var transition_message = await QueenJesiStack.speedtest_once(Config.LANG)
        ...
        // Speedtest Modules
        var user_download = await QueenJesiStack.speedtest_user()
        var user_upload = await QueenJesiStack.uploadtest_user()
        var auth_message = await QueenJesiStack.speedtest_message(Config.LANG)
        ...
        // Speedtest Modules
        var server_download = await QueenJesiStack.speedtest_server()
        var server_upload = await QueenJesiStack.uploadtest_server()
        var auth_message = await QueenJesiStack.speedtest_message(Config.LANG)
        var act_ping = end - start
...
Jesi.adCmd({pattern: 'ping$', fromMe: true, deleteCommand: false, desc: Lang.PING_DESC}, (async (message, match) => {
  var start = new Date().getTime();
...
if (Config.WORKTYPE == 'private') {
    ...
    Jesi.adCmd({pattern: 'short ?(.*)', fromMe: true, desc: Lang.URL}, (async (message, match) => {
    ...
    Jesi.adCmd({pattern: 'calc ?(.*)', fromMe: true, desc: Lang.CALC }, (async (message, match) => {
    ...
    Jesi.adCmd({pattern: 'short ?(.*)', fromMe: false, desc: Lang.URL}, (async (message, match) => {
    ...
    Jesi.adCmd({pattern: 'calc ?(.*)', fromMe: false, desc: Lang.CALC }, (async (message, match) => {
        if (match[1].length < 4) { return await message.client.sendMessage(message.jid,Lang.VALİD, MessageType.text) }
        if (match[1].includes('+')) { var split = match[1].split('+'), sonsayi = split[1], ilksayi = split[0]
            var result = -(-ilksayi - sonsayi)
            try { await message.client.sendMessage(message.jid,Lang.SUC + result, MessageType.text) }
